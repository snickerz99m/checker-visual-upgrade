...
try {
    $result = adyenFamousCheck($cardNumber, $expiry, $cvv);
    echo json_encode($result);
} catch (Exception $e) {
    echo json_encode([
        'status' => 'error',
        'message' => 'Adyen Famous Site error: ' . $e->getMessage(),
        'cardNumber' => $cardNumber
    ]);
}

function adyenFamousCheck($card, $exp, $cvv) {
    // TODO: Add your Adyen Famous Site checker logic here
    
    return [
        'status' => 'error',
        'message' => 'Adyen Famous Site checker not implemented - add your Adyen logic here',
        'cardNumber' => $card,
        'bin' => substr($card, 0, 6),
        'last4' => substr($card, -4),
